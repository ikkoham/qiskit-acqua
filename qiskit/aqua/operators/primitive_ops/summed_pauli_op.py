...
class SummedPauliOp(PrimitiveOp):
    ...
    def __init__(
        ...
        if not isinstance(primitive, SparsePauliOp):
            raise TypeError(
                "SummedPauliOp can only be instantiated with SparsePauliOp, not {}".format(
                    type(primitive)
                )
            )
    ...
    def adjoint(self) -> OperatorBase:
        return SummedPauliOp(self.primitive.conjugate(), coeff=self.coeff.conjugate())
    ...
    def equals(self, other: OperatorBase) -> bool:
        ...
        if isinstance(self.coeff, ParameterExpression) or isinstance(
            ...
            return (
                ...
                and self.primitive.simplify() == other.primitive.simplify()
            )
        simple_self = (self.coeff * self.primitive).simplify()
        simple_other = (other.coeff * other.primitive).simplify()
        return len(simple_self) == len(simple_other) and simple_self == simple_other

    def _expand_dim(self, num_qubits: int) -> "SparsePauliOp":
        return SummedPauliOp(
            self.primitive.tensor(SparsePauliOp(Pauli(label="I" * num_qubits))),
            coeff=self.coeff,
        )
    ...
    def permute(self, permutation: List[int]) -> "SparsePauliOp":
        ...
        permutation = np.hstack([permutation, permutation + length])
    ...
    def to_pauli_op(self, massive: bool = False) -> OperatorBase:
        ...
        if len(self.primitive) == 1:
            return PauliOp(
                Pauli(x=self.primitive.table.X, z=self.primitive.table.Z),
                to_native(to_real(self.primitive.coeffs[0])) * self.coeff,
            )
        return (
            SummedOp(
                [
    ...
    def simplify(self) -> "SummedPauliOp":
        return SummedPauliOp(self.primitive.simplify(), self.coeff)
